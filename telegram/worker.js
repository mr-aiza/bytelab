...
//   🖥 داشبورد (خانه) → 📋 لیدها / 🎨 گالری / 📰 بلاگ / ❓ FAQ / 📢 بنر / 🟢 وضعیت / 📊 آمار کامل / 🔍 جستجو / 📤 خروجی / 🕓 لاگ فعالیت‌ها
//   هر بخش زیرمنوی خودش رو داره و دکمه‌ی «⬅️ بازگشت» به داشبورد برمی‌گردونه.
//
// هماهنگی با پنل وب (bytelab-admin-worker.js / admin.html):
//   این وورکر از همون KV Namespace (LEADS_KV) پنل وب استفاده می‌کنه، پس همه‌ی
//   داده‌ها (لیدها، بلاگ، FAQ، گالری، بنر، وضعیت) کاملاً مشترک و هم‌زمانه.
//   لاگ فعالیت (کلید KV: admin_activity) هم بین این دو Worker کاملاً مشترکه —
//   هر اقدامی که از تلگرام یا از پنل وب انجام بشه، تو یک لاگ واحد ثبت می‌شه
//   (با فیلد source برای تشخیص مبدا) و از هر دو رابط قابل مشاهده‌ست.
//   خروجی CSV لیدها (📤 خروجی لیدها) هم دقیقاً هم‌ستون با خروجی CSV پنل وبه.
...
// تشخیص اینکه آیا خطا از نوع «سهمیه رایگان تموم شده» است یا نه؛
// اگه بله، دیگه دلیلی نداره دوباره‌دوباره تلاش کنیم (چون تا فردا درست نمی‌شه و فقط وقت/فراخوانی هدر می‌ره)
function isQuotaExceededError(err) {
  const msg = String((err && err.message) || err || "");
  return msg.includes("سهمیه") || msg.includes("4006") || msg.toLowerCase().includes("daily free allocation");
}
...
const MAX_ATTEMPTS = 3;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = err;
      // اگه سهمیه رایگان تموم شده، تلاش‌های بعدی هم همین جواب رو می‌گیرن؛ همینجا متوقف می‌شیم تا هدر نره
  ...
  // (ولی اگه دلیل شکست، تموم‌شدن سهمیه بود، این تلاش آخر هم بی‌فایده‌ست و انجامش نمی‌دیم)
  if (!parsed && !isQuotaExceededError(lastError)) {
    try {
  ...
  if (parsed) {
    ...
    await logActivity(env, "پیش‌نویس بلاگ با AI", item.title);
...
async function finishCategoryPicker(env, chatId, state, categoryStr) {
  ...
  if (next.kind === "editfield") {
    ...
    await logActivity(env, "ویرایش نمونه‌کار", item.title);
  ...
  await savePortfolioItem(env, item);
  await logActivity(env, "نمونه‌کار جدید", item.title);
...
// ============================================================
//  لاگ فعالیت (Audit Log) — دقیقاً هم‌ساختار با bytelab-admin-worker.js
//  هر دو Worker روی همون کلید KV به نام "admin_activity" می‌نویسن، پس
//  فعالیت‌های انجام‌شده از تلگرام و از پنل وب تو یک لاگ واحد و مشترک
//  دیده می‌شن (چه از تلگرام باز بشه چه از پنل وب).
// ============================================================
...
async function logActivity(env, action, detail) {
  try {
    ...
    logs.unshift({ action, detail: detail || null, time: Date.now(), source: "telegram" });
...
function activityActionIcon(action) {
  if (action.indexOf("حذف") === 0) return "🗑";
  if (action.indexOf("جدید") > -1) return "🆕";
  if (action.indexOf("ورود") === 0) return "🔑";
  if (action.indexOf("بک‌آپ") > -1 || action.indexOf("بازگردانی") > -1) return "☁️";
  return "✏️";
}
async function sendActivityLog(env, chatId, messageId) {
  const raw = await env.LEADS_KV.get("admin_activity");
  const logs = raw ? JSON.parse(raw) : [];
  const recent = logs.slice(0, 20);
  let text = "🕓 لاگ فعالیت‌ها (۲۰ مورد اخیر — از پنل وب و تلگرام)\n\n";
  if (recent.length === 0) {
    text += "هنوز فعالیتی ثبت نشده.";
  } else {
    recent.forEach((l) => {
      const date = new Date(l.time).toLocaleString("fa-IR", { timeZone: "Asia/Tehran" });
      const from = l.source === "telegram" ? "📱 تلگرام" : "💻 پنل وب";
      text += `${activityActionIcon(l.action)} ${l.action}${l.detail ? " — " + l.detail : ""}\n${from} • ${date}\n\n`;
    });
  }
  const keyboard = { inline_keyboard: [[{ text: "⬅️ بازگشت به داشبورد", callback_data: "dash:home" }]] };
  if (messageId) {
    await tgEditText(env, chatId, messageId, text.slice(0, 4000), { reply_markup: keyboard });
  } else {
    await tgSendTo(env, chatId, text.slice(0, 4000), { reply_markup: keyboard });
  }
}
...
function dashboardKeyboard() {
  return {
    inline_keyboard: [
      ...
      [
        { text: "🕓 لاگ فعالیت‌ها", callback_data: "dash:activity" },
      ],
...
async function exportLeadsFile(env, chatId) {
  ...
  const headers = ["نام", "شماره", "ایمیل", "خدمت", "نوع", "وضعیت", "تاریخ", "یادداشت"];
  const rows = leads.map((l) => {
    const date = new Date(l.createdAt).toLocaleString("fa-IR", { timeZone: "Asia/Tehran" });
    return [l.name, l.phone, l.email, l.service, l.type, statusLabel(l.status), date, l.note].map(csvEscape).join(",");
  });
  const content = "\uFEFF" + headers.map(csvEscape).join(",") + "\n" + rows.join("\n");

  const formData = new FormData();
  formData.append("chat_id", chatId);
  formData.append("document", new Blob([content], { type: "text/csv;charset=utf-8;" }), `bytelab-leads-${nowTehranDateStr()}.csv`);

  await fetch(`${TG_API(env)}/sendDocument`, { method: "POST", body: formData });
  await logActivity(env, "ساخت بک‌آپ فوری", `bytelab-backup-${nowTehranDateStr()}.json`);
}
      ...
      if (update.callback_query) {
        ...
        if (parts[0] === "restoreyes") {
          ...
          try {
            ...
            await clearAdminState(env, chatId);
            await logActivity(
              env, "بازگردانی از بک‌آپ",
              `${result.leads} لید, ${result.portfolioItems} نمونه‌کار, ${result.blogPosts} پست بلاگ, ${result.faqItems} FAQ`
            );
            await tgSendTo(
              env, chatId,
          ...
          if (raw) {
            ...
            await logActivity(env, "تغییر وضعیت لید", (lead.name || leadId) + " → " + newStatus);
            await tgEditMarkup(env, chatId, messageId, {
              inline_keyboard: [
                ...
                [{ text: "🗑 حذف لید", callback_data: `stdel:${leadId}` }],
              ],
            });
          ...
          await tgEditText(
            ...
            `⚠️ مطمئنی می‌خوای لید «${lead.name || leadId}» رو کامل حذف کنی؟ این کار قابل بازگشت نیست.`,
            {
              reply_markup: {
                inline_keyboard: [[
                  { text: "✅ بله، حذف کن", callback_data: `stdelyes:${leadId}` },
                  { text: "❌ نه، بازگشت", callback_data: `stcancel:${leadId}` },
                ]],
              },
            }
          );
          await tgAnswerCallback(env, cq.id, "");
        } else if (parts[0] === "stcancel") {
          const leadId = parts[1];
          const raw = await env.LEADS_KV.get(`lead:${leadId}`);
          if (!raw) {
            await tgAnswerCallback(env, cq.id, "این لید پیدا نشد.");
          ...
          await tgEditMarkup(env, chatId, messageId, {
            inline_keyboard: [
              [{ text: statusLabel(lead.status) + " (ثبت شد)", callback_data: "noop" }],
              [{ text: lead.note ? "📝 ویرایش یادداشت" : "📝 افزودن یادداشت", callback_data: `stnote:${leadId}` }],
              [{ text: "🗑 حذف لید", callback_data: `stdel:${leadId}` }],
            ],
          });
          await tgAnswerCallback(env, cq.id, "لغو شد");
        } else if (parts[0] === "stdelyes") {
          const leadId = parts[1];
          const raw = await env.LEADS_KV.get(`lead:${leadId}`);
          const lead = raw ? JSON.parse(raw) : null;
          await env.LEADS_KV.delete(`lead:${leadId}`);
          await logActivity(env, "حذف لید", (lead && lead.name) || leadId);
          await tgEditText(env, chatId, messageId, `🗑 لید «${lead ? lead.name || leadId : leadId}» با موفقیت حذف شد.`, {
            reply_markup: { inline_keyboard: [[{ text: "⬅️ بازگشت به داشبورد", callback_data: "dash:home" }]] },
          });
          ...
          if (item) {
            ...
            await savePortfolioItem(env, item);
            await logActivity(env, "تغییر وضعیت نمونه‌کار", item.title + " → " + newStatus);
          ...
          await logActivity(env, "حذف نمونه‌کار", item ? item.title : pfId);
          await tgEditText(
            env,
            chatId,
            messageId,
        ...
        // ---------- 🖥 داشبورد و زیرمنوها ----------
          ...
          if (action === "home") {
            ...
            await tgAnswerCallback(env, cq.id, "");
            await sendActivityLog(env, chatId, messageId);
          } else if (action === "ai") {
            if (!sub) {
              const isCustom = await isBlogSystemPromptCustom(env);
        ...
        // ---------- سوالات متداول ----------
          ...
          await logActivity(env, "حذف FAQ", item ? item.question.slice(0, 60) : parts[1]);
        ...
        // ---------- بلاگ ----------
          ...
          if (item) {
            item.status = parts[2];
            await saveBlogPost(env, item);
            await logActivity(env, "تغییر وضعیت پست بلاگ", item.title + " → " + item.status);
          ...
          await logActivity(env, "حذف پست بلاگ", item ? item.title : parts[1]);
      ...
      if (update.message && update.message.text) {
        ...
        if (state && !text.startsWith("/")) {
          ...
          if (state.mode === "leadnote") {
            ...
            await saveLead(env, lead);
            await logActivity(env, "یادداشت لید", lead.name || state.id);
            await clearAdminState(env, chatId);
          ...
          if (state.mode === "faqeditfield") {
            ...
            if (item && field) {
              ...
              await logActivity(env, "ویرایش FAQ", item.question.slice(0, 60));
              await clearAdminState(env, chatId);
          ...
          if (state.mode === "faqadd") {
            ...
            if (state.step === "question") {
              ...
              await saveFaq(env, item);
              await logActivity(env, "سوال جدید FAQ", item.question.slice(0, 60));
          ...
          if (state.mode === "blogeditfield") {
            ...
            if (item && field) {
              ...
              await saveBlogPost(env, item);
              await logActivity(env, "ویرایش پست بلاگ", item.title);
              await clearAdminState(env, chatId);
              await tgSendTo(env, chatId, `✅ «${field.label}» بروزرسانی شد.`);
              await sendBlogManageCard(env, chatId, item);
            } else {
          ...
          if (state.mode === "blogadd") {
            ...
            if (state.step === "title") {
              ...
              await saveBlogPost(env, item);
              await logActivity(env, "پست بلاگ جدید", item.title);
          ...
          sendResult = await tgSend(env, text, {
            reply_markup: {
              inline_keyboard: [
                ...
                [{ text: "🗑 حذف لید", callback_data: `stdel:${id}` }],
              ],
            },
          });
        } else {
